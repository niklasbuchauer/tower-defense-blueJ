...
public class Game extends Canvas implements Runnable
{
    ...
    private int getTowerCost()
    {
        switch (selectedTower)
        {
            case 1: return 60;   // BasicTower
            case 2: return 100;  // SniperTower
            case 3: return 80;   // RapidTower
            case 4: return 90;   // FreezeTower
        }
        return 60;
    }
    ...
    private void update()
    {
        ...
        for (Enemy e : enemies)
        {
            ...
            // Gegner hat Ziel erreicht: Leben abziehen, KEIN Geld
            if (e.reachedEnd(map.getPath()))
            {
                player.loseLife(1);
                e.setReward(0); // kein Geld, da nicht gekillt
                e.takeDamage(9999);
            }

            // Gegner wurde von Türmen gekillt: Geld geben
            if (e.isDead() && e.getReward() > 0)
            {
                player.addMoney(e.getReward());
                e.setReward(0); // verhindert doppeltes Geld
            }
        }
    ...
    private void drawMenu(Graphics g)
    {
        ...
        g.drawString("TOWER DEFENSE", 260, 250);

        g.setFont(new Font("Arial", Font.PLAIN, 18));
        g.drawString("ENTER druecken oder klicken zum Starten", 220, 300);

        g.setFont(new Font("Arial", Font.PLAIN, 14));
        g.drawString("1=Basic(60)  2=Sniper(100)  3=Rapid(80)  4=Freeze(90)", 200, 340);
        g.drawString("Linksklick = Tower setzen   Rechtsklick = Tower upgraden", 200, 360);
    }
    ...
    private void drawGameOver(Graphics g)
    {
        ...
        g.setFont(new Font("Arial", Font.PLAIN, 18));
        g.setColor(Color.BLACK);
        g.drawString("Neustart: Programm neu starten", 280, 320);
    }
    ...
    private void drawVictory(Graphics g)
    {
        ...
        g.setFont(new Font("Arial", Font.BOLD, 48));
        g.drawString("SIEG!", 330, 280);
        g.setFont(new Font("Arial", Font.PLAIN, 18));
        g.setColor(Color.BLACK);
        g.drawString("Alle Wellen besiegt!", 310, 320);
    }
